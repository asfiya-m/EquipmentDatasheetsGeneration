...
2️⃣ For each equipment name in master file (including implied ones), finds corresponding streams.
...
def populate_parameters(master_file, streamtable_file, verbose=False):
    ...
    # Parameter mapping & aggregation rules for tank
    param_mapping = {
        "Tank": {
    ...
    for _, row in equipment_rows.iterrows():
        ...
        equip_stream_map[equip_name] = stream_tags
    ...
    for sheet_name in wb_master.sheetnames:
        ...
        for col in range(4,ws.max_column +1):
            ...
            # Determine base equipment if implied (e.g., Agitator → Tank)
            streams_key = equip_name
            if sheet_name == "Agitator":
                # implied → replace 'A-' with 'TK-' to find streams
            ...
            stream_tags = equip_stream_map[streams_key]
            ...
            for stream_tag in stream_tags:
                ...
                for master_param, rule in mapping_lc.items():
                    ...
                    try:
                        ...
                        if verbose:
                            print(f" → Found {val} for stream {stream_tag}, param {master_param} (col {col_idx})")
